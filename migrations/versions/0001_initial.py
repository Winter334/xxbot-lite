...
def upgrade() -> None:
    ...
    if "ladder_records" not in table_names:
        op.create_table(
            "ladder_records",
            sa.Column("character_id", sa.Integer(), nullable=False),
    ...
    inspector = sa.inspect(bind)
    table_names = set(inspector.get_table_names())
    if "players" in table_names and not _index_exists(inspector, "players", op.f("ix_players_discord_user_id")):
        op.create_index(op.f("ix_players_discord_user_id"), "players", ["discord_user_id"], unique=True)
    if "characters" in table_names and not _index_exists(inspector, "characters", op.f("ix_characters_current_ladder_rank")):
        op.create_index(op.f("ix_characters_current_ladder_rank"), "characters", ["current_ladder_rank"], unique=False)
    if "characters" in table_names and not _index_exists(inspector, "characters", op.f("ix_characters_player_id")):
        op.create_index(op.f("ix_characters_player_id"), "characters", ["player_id"], unique=True)
    if "artifacts" in table_names and not _index_exists(inspector, "artifacts", op.f("ix_artifacts_character_id")):
        op.create_index(op.f("ix_artifacts_character_id"), "artifacts", ["character_id"], unique=True)
    if "ladder_records" in table_names and not _index_exists(inspector, "ladder_records", op.f("ix_ladder_records_character_id")):
        op.create_index(op.f("ix_ladder_records_character_id"), "ladder_records", ["character_id"], unique=True)
    if "ladder_records" in table_names and not _index_exists(inspector, "ladder_records", op.f("ix_ladder_records_rank")):
