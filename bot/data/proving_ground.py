...
"""普通怪三维难度系数（按所在层）。整体下调约 12-15%，前期降幅更大以缓解新手压力。"""
...
PG_ELITE_MULTIPLIER_RANGE = (1.20, 1.38)
"""精英怪相对于同层普通怪的数值倍率范围（已下调约 8%）。"""
...
PG_INVEST_STAT_BOOST_PCT_PER_LEVEL = 7
"""每级"强化体魄"提供的三维加成百分比（满级 +70%，从原 +40% 大幅提升）。"""

PG_INVEST_STAT_BOOST_MAX_LEVEL = 10
"""强化体魄最大等级。"""

PG_INVEST_STAT_COSTS: tuple[int, ...] = (
    300, 600, 1_200, 2_500, 4_500,
    7_500, 12_000, 20_000, 32_000, 50_000,
)
"""强化体魄第 1~10 级各自的灵石花费（已下调约 35-40%，让玩家更快堆满）。"""

PG_INVEST_AFFIX_SLOT_MAX = 2
"""初始词条槽位最大解锁数。"""

PG_INVEST_AFFIX_COSTS: tuple[int, ...] = (8_000, 22_000)
"""解锁第 1、2 个初始词条槽位的灵石花费（下调约 45%）。"""

PG_INVEST_SPIRIT_COST: int = 18_000
"""解锁初始器灵槽位的灵石花费（从 30_000 降至 18_000）。"""


# -- 命数机制 --

PG_INITIAL_LIVES: int = 3
"""玩家进入证道战场时的初始命数。每次战败扣 1 条，扣光才算最终失败。"""
